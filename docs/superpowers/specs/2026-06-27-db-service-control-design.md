...
- **Date:** 2026-06-27
- **Status:** Review-revised (minor fixes applied 2026-06-27)
...
POST /admin/databases/service
  ...
  DbServiceController::action()
    ...
    DbServiceOperationJob::dispatch($operation, $engine, $action)   ← no $service param
    return response()->json(['operation_id' => $operation->id])

DbServiceOperationJob (extends OperationJob):
  run(callable $emit):
    $service = config('laranode.db_engines')[$this->engine]['service']  ← resolved in job
    $emit("Running: systemctl {$this->action} {$service}...")
    Process::run(['sudo', bin_path.'/laranode-db-service.sh', $this->action, $this->engine])
...
**Postgres variant:** on Ubuntu 24.04, `postgresql@16-main` is the active unit, not the `postgresql` alias. The script tries `postgresql` first; on non-zero exit it retries `postgresql@16-main` — but the retry itself is wrapped in `if ! ... ; then` so a retry failure also emits an explicit human-readable error before exiting. This mirrors the `$extraCandidates` pattern in `EngineManager`.
...
> **Security note — `(ALL)` run-as:** This deviates from the constraint stated in the system prompt ("run-as only the intended user, not ALL"). However, `(ALL)` is consistent with every other existing entry in `laranode-panel`. Changing to `(root)` would harden this entry, but doing so selectively while leaving all other entries at `(ALL)` provides no meaningful security improvement and would introduce inconsistency. For v1 this is accepted; the script itself is the security boundary — it validates all inputs and only calls `systemctl` on the resolved service name. This inconsistency is documented here for future hardening consideration (narrowing all sudoers entries to `(root)` in a single pass).
...
Extends the abstract `OperationJob`. Constructor receives `Operation $operation`, `string $engine`, `string $action` **only** — `$service` is intentionally excluded and resolved inside `run()` from config. This eliminates the engine/service mismatch footgun: no caller can pass `engine='mysql'` with `service='sshd'`.
...
class DbServiceOperationJob extends OperationJob
{
    ...
    {
        // Resolve service name from config — never accept it from the caller.
        ...
        $result = Process::run([
            ...
            $this->action,
            $this->engine,   // engine key only — script resolves service internally
        ]);
        $emit($result->output());
        if ($result->failed()) {
...
> **Note — operation target log:** `target` is stored as `'{engine}:{service}'` where `$service` is resolved in the controller from config before dispatching. If config changes between the controller and job execution (e.g., a mid-flight deploy), the logged target could be stale, though the actual `systemctl` call (resolved inside the script) remains correct. Extremely low probability; acceptable for v1.

### `App\Services\Database\DbServiceStatusService` (new, `app/Services/Database/DbServiceStatusService.php`)

Retrieves per-engine service status for the UI. Injected with `EngineManager`. Called by `DbServiceController::status()`.
...
> **Note — EngineManager binding:** `EngineManager` is bound as transient (not singleton) in the Laravel DI container. The memoization in `available()` is per-instance and works correctly within a single request, since `DbServiceController` receives one `EngineManager` instance for the lifetime of that request.
...
class DbServiceController extends Controller
{
    ...
    public function action(DbServiceRequest $request): JsonResponse
    {
        ...
        // Resolve service name from config (used only for the audit target column).
        $engines = config('laranode.db_engines');
        ...
        // $service is NOT passed to the job — the job resolves it from config itself.
...
3. On button click, calls `axios.post(route('databases.service.action'), { engine, action })` which returns `{ operation_id }`. A `catch` block resets `loading` to `false` and surfaces the error so buttons do not stay permanently disabled on HTTP error.
...
if ! systemctl "$ACTION" "$SERVICE"; then
    # For postgres, also try versioned unit (Ubuntu 24.04 default: postgresql@16-main).
    # Keep in sync with EngineManager::$extraCandidates in app/Databases/EngineManager.php
...
4. **Job constructor isolation**: `DbServiceOperationJob` does NOT accept `$service` as a constructor parameter. The job resolves the service name from `config('laranode.db_engines')[$this->engine]['service']` internally. This eliminates the possibility of a caller passing a mismatched engine/service pair, and ensures the engine key (not the raw service name) is the only identifier that crosses the job dispatch boundary.

5. **Leading-dash injection**: both PHP (`Rule::in` with a closed enum) and the script (`[[ "$ARG" == -* ]]` guard + `case`) reject arguments starting with `-`. This prevents `--foo` flag injection to `systemctl`.

6. **No arbitrary service control**: the script will never call `systemctl start sshd` or any other service not in the case block. The mapping from engine to service name lives in the script — PHP passes `mysql`, the script calls `mysql`.

7. **Queue worker isolation**: `DbServiceOperationJob` runs in the queue worker process (the `laranode-queue-worker.service` systemd unit), not in the web request. If the web process is compromised, the queue worker limits blast radius to the jobs it already processes.

8. **sudoers scope**: `www-data ALL=(ALL) NOPASSWD:` for the specific script path only — no wildcard on arbitrary commands. See sudoers note above regarding the `(ALL)` run-as deviation.

9. **Audit trail**: every action creates an `Operation` row visible to all admins at `/admin/operations`, with `type`, `target`, `output`, and `started_at` / `finished_at` timestamps.

10. **Stop safety**: stopping a DB engine that has active panel connections will cause subsequent requests to fail until the service is restarted. This is expected and intentional for admin control — no additional guard is needed in v1. A future improvement could warn if active connections are detected.

## Testing Strategy

### Pest unit tests — system-gated (`tests/Feature/Database/DbServiceSystemTest.php`)

Table-driven tests for argument validation that the script enforces. Gated with `LARANODE_SYSTEM_TESTS=1`. For the non-system suite, the PHP-layer allowlist tests in `DbServiceControllerTest` / `DbServiceRequestTest` provide coverage.
...
- **Admin gate (action)**: `POST /admin/databases/service` as non-admin → 403.
- **Admin gate (status)**: `GET /admin/databases/service/status` as non-admin → 403.
...
- **Engine allowlist rejection**: `engine=invalid_engine` → 422.
- **Engine leading-dash**: `engine=-mysql` → 422.
- **Action allowlist rejection**: `action=nuke` → 422.
- **Action leading-dash**: `action=--help` → 422.
- **Successful dispatch**: valid `{engine: 'mysql', action: 'restart'}` → 200 JSON with `operation_id`; `Operation` row exists with `type='db.service.restart'`, `target='mysql:mysql'`, `status='queued'`; `DbServiceOperationJob` dispatched (via `Queue::fake()` / `Bus::fake()`).
- **Status endpoint**: `GET /admin/databases/service/status` as admin → JSON with each configured engine and its `active` state (mocked via `Process::fake()`).

### Pest feature tests (`tests/Feature/Database/DbServiceOperationJobTest.php`) — new

Uses `Process::fake()`:

- **Success path**: `DbServiceOperationJob::dispatchSync($op, 'mysql', 'restart')` → `$op->status === 'succeeded'`; `$op->output` contains "Running:" and "completed."; exit code 0.
- **Failure path**: `Process::fake()` returns exit code 1 → `$op->status === 'failed'`; job re-throws (recorded in failed_jobs).
- **Output capture**: assert `$op->fresh()->output` contains the faked stdout string.
- **Correct args to `Process::run` (mysql)**: command array contains `'laranode-db-service.sh'`, `'restart'`, and `'mysql'` (the engine key — same as service name for mysql, but proves only the engine key is passed).
- **Correct args to `Process::run` (postgres)**: `DbServiceOperationJob::dispatchSync($op, 'postgres', 'restart')` → assert command array third arg is `'postgres'` (not `'postgresql'`), proving the job never passes the raw service name to the script.
- **Output does not leak service name (postgres)**: for `engine='postgres'`, assert `$op->fresh()->output` does NOT contain the literal string `'postgresql'` — the raw service name is the script's internal concern only.
...
- **Test 1 (script allowlist)**: `laranode-db-service.sh start invalid_engine` → exit non-zero.
...
- **Test 4b (restart postgres — covers retry branch)**: `laranode-db-service.sh restart postgres` → exit 0; container must have PostgreSQL installed; exercises the `postgresql@16-main` fallback branch in the script.
- **Test 5 (status endpoint round-trip)**: `GET /admin/databases/service/status` as admin → `statuses.mysql.active === true`.
...
- **Buttons disabled during operation**: after click (before `onDone`), all buttons have `disabled` attribute until `onDone` fires.
...
- **Error path (axios reject)**: mock `axios.post` to reject with a 422 error → assert `loading` is reset to `false` (buttons re-enabled) and error message is rendered. This covers the "buttons stuck disabled forever" bug on HTTP error.
...
tests/Feature/Database/DbServiceRequestTest.php                          (new)
...
| # | Issue | Fix |
|---|-------|-----|
| 1 | Sudoers `(ALL)` run-as deviates from stated system prompt constraint | Added explicit security note documenting the deviation, its rationale, and future hardening path |
| 2 | Postgres retry branch: `set -e` + bare `systemctl` retry = no error message on retry failure | Wrapped retry in `if ! systemctl "$ACTION" "postgresql@16-main"; then ... exit 1; fi` with explicit `echo "ERROR: ... also failed" >&2` |
| 3 | `DbServiceOperationJob` constructor accepted `$service` as param — engine/service mismatch footgun | Dropped `$service` from constructor; job self-resolves via `config('laranode.db_engines')[$this->engine]['service']` |
| 4 | `doAction` in JSX had no `catch` — any HTTP error permanently disabled all buttons | Added `try/catch`; on error: `setLoading(false)` + `setError(...)` displayed inline |
| 5 | Script's postgres case and retry had no comment pointing to `EngineManager.php` | Added `# Keep in sync with EngineManager::$extraCandidates` comments in both locations |
| 6 | No test for postgres engine arg (engine key vs service name distinction) | Added job test asserting third Process::run arg is `'postgres'` not `'postgresql'`; added output-does-not-leak test |
| 7 | No system test for postgres retry branch | Added Test 4b: `laranode-db-service.sh restart postgres` in container |
| 8 | No Vitest test for `doAction` error path | Added error-path test: axios reject → loading false + error rendered |
| 9 | Unauthenticated → 302 only tested for status endpoint; missing for action endpoint | Added unauthenticated `POST /admin/databases/service` → 302 test |
...
2. **Postgres versioned unit (postgresql@16-main)**: the retry logic handles PG 16 (Ubuntu 24.04 default). A PG 17 install would need `postgresql@17-main`. The EngineManager has `$extraCandidates` for this; the script is a manual copy. If PG 17 support is added to EngineManager, the script must be updated (the sync comments help). An alternative — accepting an optional third arg `<service-override>` — would change the interface and remove the defense-in-depth benefit.

3. **Job queue**: `DbServiceOperationJob` runs on the default queue (`QUEUE_CONNECTION=database`, `laranode-queue-worker.service`). Should it run on a dedicated `db-service` queue to avoid a slow restart blocking backup or SSL jobs?

4. **MariaDB vs MySQL coexistence**: `DbServiceStatusService` lists all configured engines regardless of whether they are installed. This could show Start/Stop buttons for engines not present on the host. Should the UI hide buttons for engines where `systemctl is-active` returns `Unit not found` vs. `inactive`?

5. **Restart vs reload**: `systemctl reload` applies config changes without dropping connections. Should a "Reload" action be added? Deferring to v2 requires only one more allowed action string and script branch.
